    ...
    def __init__(
        ...
        for name, value in attr_map.items():
            if isinstance(value, str):
                self.attr_map[name] = self.get_json_mapper_func(value)
        ...
        if isinstance(iterate_nodes, str):
            iterate_expression = jsonpath_ng.parse(iterate_nodes)
            iterate_nodes = iterate_expression.find

        self.iterate_nodes = iterate_nodes

    @staticmethod
    def get_json_mapper_func(jp_value):
        ...
        jp_expr = jsonpath_ng.parse(jp_value)
