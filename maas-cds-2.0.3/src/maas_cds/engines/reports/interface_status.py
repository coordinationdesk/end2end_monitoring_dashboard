...
class InterfaceStatusConsolidatorEngine(RawDataEngine):
    ...
    def consolidate_from_InterfaceProbe(
        ...
        if next_document:
            ...
            if self.status_contains_probe(next_document, raw_document):

                return self.split_status(next_document, raw_document)

            # else
            next_document.status_time_stop = raw_document.probe_time_start
            next_document.calculate_duration()
            next_document.save()
