...
class AcquisitionPassStatusConsolidatorEngine(
    AnomalyImpactMixinEngine, HktmImpactMixinEngine, ReplicatorEngine
):
    """
    ...
    def action_iterator(self) -> Iterator[Dict[str, Any]]:
        ...
        if self.payload.document_class == "ApsSession":
            yield from self.update_hktm(
                self.payload.document_class, self.consolidated_documents
            )
...
class XBandAcquisitionPassStatusConsolidatorEngine(
    ...
    def __init__(self, args, exclude_fields=None, send_reports=False, min_doi=None):
        super().__init__(
            args,
